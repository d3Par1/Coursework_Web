...
// Idempotently creates a demo user with realistic data for defense demo.
...
//   password: demo123
//
// Wipes ONLY the demo user's data. Other users (including real ones) untouched.
...
const wipeAndSeed = db.transaction(() => {
  ...
  return { userId, txCount: transactions.length };
});

const result = wipeAndSeed();
...
console.log(`  Transactions: ${result.txCount} across last 35 days`);
console.log('');
